...
graduates.forEach(graduates => {
  ...
  contactInfo.sort(); // Sorted because why not :thumbsup:
});
...
const studentInfo = arr => {
  arr.forEach(arr => {
    contactInfo.push(`${arr.first_name} ${arr.email}`);
    contactInfo.sort();
  });
  console.log(contactInfo);
};
studentInfo(graduates); // Sorted because why not :thumbsup:
*/
...
console.log(
  `There are ${unisWithUni.length} university's with the tag "Uni" in them`
);
...
zooAnimals.forEach(zooAnimals => {
  displayNames.push(
    `Name: ${zooAnimals.animal_name}, Scientific: ${zooAnimals.scientific_name}`
  );
  displayNames.sort(); // Sorted because why not :thumbsup:
});
...
const lowCaseAnimalNames = zooAnimals.map(function(arr) {
...
*/
const lowPopulationAnimals = zooAnimals.filter(function(arr) {
...
}, 0); // This damn index param!!!!! Optional my A$$!

console.log(`The Zoo's current total population is ${populationTotal}`);
